...
export default async function HumorFlavorStepsPage() {
  ...
  const { data: steps } = await supabase.from("humor_flavor_steps").select("*").order("created_at", { ascending: false });

  const keys = steps && steps.length > 0 ? Object.keys(steps[0]) : ["id", "humor_flavor_id", "step_order", "description", "created_at"];

  return (
    <main style={{ minHeight: "100vh", background: "#f5f5f5" }}>
      <NavHeader userEmail={user.email} />
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
        <h2 style={{ margin: "0 0 20px", fontSize: 18, fontWeight: 600 }}>🪜 Humor Flavor Steps ({steps?.length ?? 0})</h2>
